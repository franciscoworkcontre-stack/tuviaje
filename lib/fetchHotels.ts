...
import { fetchScraperHotels, type ScraperHotel } from "./scraperClient";
...
async function scrapeCity(
  ...
  // with the existing scoring formula (bayesRating expects 0-10)
  let pool = raw
    .filter(h => h.price_per_night > 0)
    .map(h => ({
      ...h,
      ratingTen: (h.rating ?? 0) * 2,           // 0-5 → 0-10
  ...
  const capUsd = capClp ? capClp / USD_TO_CLP : Infinity;
  if (pool.length >= 2) {
    const capped = pool.filter(h => h.price_per_night <= capUsd);
  ...
  const scored = pool
    .map(h => {
      const priceClp = Math.round(h.price_per_night * USD_TO_CLP);
      return { h, priceClp, score: hotelScore(priceClp, h.ratingTen, h.reviews, travelStyle) };
    })
  ...
  const candidates: HotelCandidate[] = scored.map((s, i) => ({
    ...
    stars: 0,   // not available from scraper
    rating: s.h.ratingTen,
    reviews: s.h.reviews,
  ...
  const rating  = winner.ratingTen;
  const reviews = winner.reviews;
  const stars   = travelStyle === "premium" ? 5 : travelStyle === "comfort" ? 4 : 3;
