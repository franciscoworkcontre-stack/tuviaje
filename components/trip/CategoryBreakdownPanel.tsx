  ...
  switch (category) {
    ...
    case "accommodation": {
      ...
      for (const city of trip.cities) {
        ...
        if (hotel) {
          const total = hotel.pricePerNightClp * city.days;
          items.push({
            label: hotel.name,
          ...
          // Fallback to trip.accommodations
    ...
    case "localTransport": {
      const items: BreakdownItem[] = trip.days
    ...
    case "extras": {
      // extras is a lump sum — show per-day breakdown
      const perDay = trip.costs.extras > 0
        ? Math.round(trip.costs.extras / trip.totalDays)
        : 0;
      if (perDay === 0) return [];
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[9px] text-[#B0BEC5] mt-0.5">
                          {Math.round(barPct)}% de esta categoría
                        </p>
                      </div>
                    </div>
                  </motion.div>
